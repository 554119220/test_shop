...
class GoodsApplyNaming
{
	...
	function index()
	{
		try {
			...
			if ( isset($param['state']) && preg_match('/^[0-9]+$/', $param['state']) ) {
				$map['goods_apply_naming_is_audit'] = intval($param['state']);
			}
			$list = Fun::pageList(Fun::mApi('goods','GoodsApplyRecommend1'),[
				'order'		=> 'goods_apply_naming_update_time desc',
				'where' 	=> $map,
		...
		try {
			...
			$auditList = Fun::dataAll(Fun::mApi('goods','GoodsApplyRecommend1'),[
				'where' => [
					...
					'goods_apply_naming_is_audit'	=> State::STATE_DISABLED,
				],
				'field' => 'goods_id',
			]);
			# get data
	...
	function create()
	{
		try {
			...
			# 超出今日申请数量
			$nums_key = \app\common\traits\Cache::getCacheName(Cache::GOODS_APPLY_naming_NUMS) . $user_id;
			...
			if ( $apply ) {
				...
				# 修改记录
				$map = [
					'goods_apply_naming_id' => $apply['goods_apply_naming_id'],
				];
				...
				db('goods_apply_naming')->where($map)->update(['goods_apply_naming_create_time' => time()]);
			} else {
				# 增加记录
