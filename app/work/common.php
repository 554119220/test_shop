...
if (!function_exists('thumb')) {
    function thumb($url,$param=null,$h='',$t=2,$nopic='',$type=''){
        $nopic = $nopic ? $nopic : '/static/images/nopic_200x200.png';


        return \app\common\traits\F::thumbnail($url, $param, $h);
 ...
 * @param integer $height 图片高度
 */
function imgwh2($url,$width=80,$height='',$type=''){
    return '<a class="image-zoom" href="' . \app\common\traits\F::getImagesDomain() . $url.'" title="大图"><img src="'.thumb($url,$width,$height,2,'',$type).'" alt="图片"></a>';
}
/**
 * 终端格式为图片输出
 * @param $val
 ...
 * @return 如：大分类 > 小分类
 */
function nav_sort2($value,$options){
    $nav = [];
    $where[$options['pk'] ?? 'id']    = $value;
    $field          = $options['field'] ? $options['field'] : 'id,upid,category_name';

    $cache_name     = md5('nav_sort' . serialize([$options['table'],$value,$where,$field]));
    $cache_time     = 1800;
    $rs = db($options['table'])->cache($cache_name,$cache_time)->where($where)->field($field)->find();

    if($rs){
        $nav[] = $rs[$options['key'][1]];
        if($rs[$options['upkey'] ?? 'upid'] > 0) {
            $tmp = nav_sort2($rs[$options['upkey']],$options);
            $nav = array_merge([$tmp],$nav);
        }
    }
...
function getunserialize($val,$str2='',$str=''){
    if (is_string($val)){
        $data = unserialize($val);
    }else{
        return '';
    }
    return $data[$str2][$str] ?? '';
}
