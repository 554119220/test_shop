...
//        if (!empty($config['auto_start'])) {
    ...
    public static function delete($name, $prefix = null)
    {
        empty(self::$init) && self::boot();
        $prefix = !is_null($prefix) ? $prefix : self::$prefix;
        F::writeLogByMongoDb('test',['status'=>1,'type'=>'logout','name'=>$name,'time'=>date('Y-m-d H:i:s')]);
        if (is_array($name)) {
            F::writeLogByMongoDb('test',['status'=>2,'type'=>'logout','name'=>$name,'time'=>date('Y-m-d H:i:s')]);
            foreach ($name as $key) {
                self::delete($key, $prefix);
                F::writeLogByMongoDb('test',['status'=>3,'type'=>'logout','name'=>$name,'key'=>$key,'time'=>date('Y-m-d H:i:s')]);
            }
        } elseif (strpos($name, '.')) {
            F::writeLogByMongoDb('test',['status'=>4,'type'=>'logout','name'=>$name,'time'=>date('Y-m-d H:i:s')]);
            list($name1, $name2) = explode('.', $name);
            if ($prefix) {
            ...
            F::writeLogByMongoDb('test',['status'=>5,'type'=>'logout','name'=>$name,'prefix'=>$prefix,'time'=>date('Y-m-d H:i:s')]);
            if ($prefix) {
                unset($_SESSION[$prefix][$name]);
                F::writeLogByMongoDb('test',['status'=>6,'type'=>'logout','name'=>$name,'prefix'=>$prefix,'time'=>date('Y-m-d H:i:s')]);
            } else {
                unset($_SESSION[$name]);
                F::writeLogByMongoDb('test',['status'=>7,'type'=>'logout','name'=>$name,'prefix'=>$prefix,'time'=>date('Y-m-d H:i:s')]);
            }
        }
    }
