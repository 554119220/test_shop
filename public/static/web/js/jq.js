...
$(function(){
	...
	$(document).on('click','.help_left .nav a',function(){
			$('.help_left .nav a').removeClass('color-red1');
			$(this).addClass('color-red1');
		})
	...
	$(document).on('click','.class-btn',function(){
		...
		$('.bg-000-f').hide()
		//$(' mb5').eq(n).val($('.class-active').eq(0).attr('title')+' > '+$('.class-active').eq(1).attr('title'))
		html = $('.bg-000-f').eq(n).find('.class-active').eq(0).find('.active').text()+' > '+$('.bg-000-f').eq(n).find('.class-active').eq(1).find('.active').text()		
		html = '<div class="alert alert_defelt_set" role="alert"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>'+html+'</div>'
		$('.append_input').eq(n).append(html)
	})
