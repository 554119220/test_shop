...
class HMongodb {
     ...
     * 失败：false
     */
    public function __construct($config=[], $connect=true, $auto_balance=true)
    {
        $config = array_merge(Config::get('mongodb'),$config);
        $host = 'mongodb://'.$config['username'].':'.$config['password'].'@'.$config['hostname'].':'.$config['hostport'].'/'.$config['database'];

        try {
            //$host = 'mongodb://ylsc:ylsc654321@192.168.3.205:27017/ylscdb';
            //$this->mongo = new \MongoClient($mongo_server, array('connect'=>$connect));
            $this->mongo = new \MongoClient($host, array('connect'=>$connect));
        }
